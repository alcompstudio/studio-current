...
export default function EditOptionFormSimple({
  ...
  useEffect(() => {
    const initialPricingTypeId = option.pricing_type_id || (option.pricing_type === 'calculable' ? 1 : 2);
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      if (name === 'pricing_type_id' || !name) {
        ...
        console.log('Изменение типа ценообразования:', pricingTypeId, typeof pricingTypeId);
  ...
  const onSubmit = async (data: OptionFormValues) => {
    try {
      ...
      if (calculableFieldsVisible && data.nominal_volume && data.price_per_unit) {
