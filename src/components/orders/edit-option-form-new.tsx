...
const optionFormSchema = z.object({
  ...
  (data) => {
    ...
    if (data.pricing_type_id === 1) {
      return data.nominal_volume !== null && 
  ...
  {
    message: "Для калькулируемых опций требуется указать номинальный объем и цену за единицу",
...
export default function EditOptionForm({
  ...
  const form = useForm<OptionFormValues>({
    ...
    defaultValues: {
      ...
      pricing_type_id: option.pricing_type_id || (option.pricing_type === 'calculable' ? 1 : 2), // Поддержка обратной совместимости
