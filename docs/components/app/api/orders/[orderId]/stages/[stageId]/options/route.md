...
*Путь к файлу: `src/app\api\orders\[orderId]\stages\[stageId]\options\route.ts`*
...
flowchart TD
    ...
    db[@/lib/db]
    route_ts --> db
    StageOption[@/lib/models/StageOption]
    route_ts --> StageOption
    OrderStage[@/lib/models/OrderStage]
    route_ts --> OrderStage
    pg[pg]
    route_ts --> pg
```
...
*Источник: `src/app\api\orders\[orderId]\stages\[stageId]\options\route.ts`*
